...
def compute_hessians_trace(model, loss, maxIter=100, tol=1e-6):
...
def normalization(vs):
    ...
    norms = sum([torch.sum(v*v) for v in vs])
    norms = (norms**0.5).cpu().item()
    vs = [vi / (norms + 1e-6) for (i, vi) in enumerate(vs)]
    return vs
...
def compute_eigenvalue(model, loss, maxIter=100, tol=1e-8, top_n=1):
    # Get parameters and gradients of corresponding layer
    weights, names = get_weights(model)
    model.zero_grad()
...
def main(args):
    ...
    for step, batch in enumerate(train_loader):
        ...
        layer_hessian_traces = compute_hessians_trace(model, loss)
        layer_hessian_eigenvalues, _ = compute_eigenvalue(model, loss, top_n=1) 

        hessian_traces.append(layer_hessian_traces)
        hessian_lambdas.append(layer_hessian_eigenvalues[0])

        print(np.mean(np.array(hessian_traces), axis=0))
        print(np.mean(np.array(hessian_lambdas), axis=0))
